...
const confirmarEleccion = () => {
    if (!ofertaSeleccionada.value || procesando.value) return;
    ...
    router.post(route('ofertas.elegir', ofertaSeleccionada.value.id), {}, {
        ...
        onError: (errors) => {
            procesando.value = false;
            errorMensaje.value = errors.error || 'Error al procesar la elección';
            console.error('Error al elegir oferta:', errors);
        },
        onFinish: () => {
            procesando.value = false;
        },
...
const generarOrdenCompra = () => {
    ...
    router.visit(route('ordenes.create', { oferta_id: ofertaElegida.value?.id }));
};
            ...
            <div v-if="mostrarModalConfirmacion" 
                ...
                <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
                    ...
                    <div class="relative bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-750 px-6 py-4 flex flex-col gap-3">
                            <!-- Mensaje de error -->
                            <div v-if="errorMensaje" class="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 p-3 rounded-md">
                                {{ errorMensaje }}
                            </div>
                            
                            <div class="flex flex-col sm:flex-row gap-3 justify-end">
                                <button 
                                    @click="mostrarModalConfirmacion = false; errorMensaje = null"
                                    :disabled="procesando"
                                ...
                                <button 
                                    ...
                                    :disabled="procesando"
                                    class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-md transition-colors disabled:opacity-50 flex items-center justify-center">
                                    ...
                                    {{ procesando ? 'Procesando...' : 'Confirmar Elección' }}
                                </button>
                            </div>
                        </div>
                    </div>
