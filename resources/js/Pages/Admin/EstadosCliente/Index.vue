...
// Formulario adaptado a EstadoCliente (nombreEstado, estadoClienteID)
const form = useForm({
...
const openCreateModal = () => {
    ...
    modalTitle.value = 'Nuevo Estado de Cliente';
    form.reset();
    form.clearErrors();
...
const openEditModal = (estado) => {
    ...
    form.estadoClienteID = estado.estadoClienteID;
...
const submit = () => {
    if (isEditing.value) {
        ...
        form.put(route('admin.estados-cliente.update', form.estadoClienteID), {
            onSuccess: () => closeModal()
        });
    } else {
        form.post(route('admin.estados-cliente.store'), {
            onSuccess: () => closeModal()
        });
    }
};
...
const deleteEstado = (estado) => {
    if (confirm(`¿Estás seguro de eliminar el estado "${estado.nombreEstado}"?`)) {
        router.delete(route('admin.estados-cliente.destroy', estado.estadoClienteID));
    }
};
    ...
    <AppLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Estados de Cliente</h2>
                    ...
                    <div class="p-6">
                        <table class="min-w-full divide-y divide-gray-200 mb-4">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="estado in estados.data" :key="estado.estadoClienteID">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">#{{ estado.estadoClienteID }}</td>
                    ...
                    <div class="px-6 py-4 border-t border-gray-200 bg-gray-50" v-if="estados.links">
                        <div class="flex justify-center items-center space-x-1">
                            <template v-for="(link, k) in estados.links" :key="k">
                                ...
                                <span v-else class="px-3 py-1 min-w-[32px] text-center border rounded text-sm bg-gray-100 text-gray-400" v-html="getPaginationLabel(link.label, k, estados.links.length)"></span>
                            </template>
                        </div>
                    </div>
                </div>
        ...
        <Modal :show="showModal" @close="closeModal">
            <div class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <TextInput id="nombreEstado" v-model="form.nombreEstado" type="text" class="mt-1 block w-full" placeholder="Ej: En Legales" />
