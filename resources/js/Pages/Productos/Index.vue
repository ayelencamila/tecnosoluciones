...
const proveedoresOptions = computed(() => [
    ...
    ...props.proveedores.map(p => ({ value: p.id, label: p.razon_social }))
]);
...
const getEstadoBadgeClass = (estadoNombre) => {
    switch (estadoNombre?.toLowerCase()) {
        ...
        case 'descontinuado': return 'bg-orange-100 text-orange-800';
...
const getPrecioMinorista = (precios) => {
    ...
    return precio ? parseFloat(precio.precio).toLocaleString('es-AR', { style: 'currency', currency: 'ARS' }) : '-';
};
                            ...
                            <thead class="bg-gray-50">
                                <tr>
                                    <th @click="sortBy('nombre')" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100 select-none">
                                        Producto
                                        <span v-if="form.sort_column === 'nombre'">{{ form.sort_direction === 'asc' ? '↑' : '↓' }}</span>
                                    </th>
                                    ...
                                    <th @click="sortBy('estadoProductoID')" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100 select-none">
                                        Estado
                                        <span v-if="form.sort_column === 'estadoProductoID'">{{ form.sort_direction === 'asc' ? '↑' : '↓' }}</span>
                                    </th>
                                ...
                                <tr v-if="productos.data.length === 0">
                                    <td colspan="6" class="px-6 py-12 text-center text-gray-400">
                                        <div class="flex flex-col items-center justify-center">
                                            <svg class="w-12 h-12 mb-3 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                            <span class="text-lg font-medium">No se encontraron productos</span>
                                            <span class="text-sm mt-1">Intenta ajustar los filtros</span>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
        ...
        <Modal :show="confirmingDeletion" @close="closeModal">
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900">
                    ¿Dar de baja el producto "{{ productToDelete?.nombre }}"?
                </h2>

                <p class="mt-1 text-sm text-gray-600">
                    Esta acción cambiará su estado a "Inactivo". Por favor, ingrese el motivo.
                </p>
